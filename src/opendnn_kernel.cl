...
void matmul_block_lin_shared(
    ...
    int Row = by*BLOCK_SIZE + thy;
    int Col = bx*BLOCK_SIZE + thx;

    local float As[BLOCK_SIZE*BLOCK_SIZE];
    local float Bs[BLOCK_SIZE*BLOCK_SIZE];

    for (int k = 0; k < (ACols + BLOCK_SIZE-1)/BLOCK_SIZE; k++) {
        if (k*BLOCK_SIZE + thx < ACols && Row < ARows)
            As[thy*BLOCK_SIZE + thx] = A[Row*ACols + k*BLOCK_SIZE + thx];
        else
            As[thy*BLOCK_SIZE + thx] = 0.0f;

        if (k*BLOCK_SIZE + thy < BRows && Col < BCols)
            Bs[thy*BLOCK_SIZE + thx] = B[Col + (k*BLOCK_SIZE + thy)*BCols];
        else
            Bs[thy*BLOCK_SIZE + thx] = 0.0f;

        barrier(CLK_LOCAL_MEM_FENCE);

        for (int n = 0; n < BLOCK_SIZE; ++n)
            CValue += (As[thy*BLOCK_SIZE + n] * Bs[n*BLOCK_SIZE + thx]);
...
void matmul_block_lin_shared_trans(
    ...
    for (int k = 0; k < (ACols + BLOCK_SIZE-1)/BLOCK_SIZE; k++) {
        ...
        if (k*BLOCK_SIZE + thy < BCols && Col < BRows)
            Bs[thy*BLOCK_SIZE + thx] = B[BCols*Col + k*BLOCK_SIZE + thy];
        else
            Bs[thy*BLOCK_SIZE + thx] = 0.0f;
...
void matmul_block_lin_shared_batch(
    ...
    for (int k = 0; k < (ACols + BLOCK_SIZE-1)/BLOCK_SIZE; k++) {
        ...
        if (k*BLOCK_SIZE + thy < BRows && Col < BCols)
            Bs[thy*BLOCK_SIZE + thx] = B[bz*BCols*BRows*group + (k*BLOCK_SIZE + thy)*BCols + Col];
        else
            Bs[thy*BLOCK_SIZE + thx] = 0.0f;
