...
// Blocked (Tiled) & linearized shared memory
__global__ void number_matmul_block_lin_shared(float* A, float* B, float* C,
                         int ARows, int ACols, int BRows,
                         int BCols, int CRows, int CCols, Number::TypeInfo metaA, Number::TypeInfo metaB)
{
    float CValue = 0;
    ...
    __shared__ float As[BLOCK_SIZE*BLOCK_SIZE];
    __shared__ float Bs[BLOCK_SIZE*BLOCK_SIZE];
    Number tempA(metaA.get_type(), metaA.get_bwTotal(), metaA.get_bias());
    Number tempB(metaB.get_type(), metaB.get_bwTotal(), metaB.get_bias());
    ...
    for (int k = 0; k < (BLOCK_SIZE + ACols - 1)/BLOCK_SIZE; k++) {
        ...
        for (int n = 0; n < BLOCK_SIZE; ++n){
            tempA.init(As[threadIdx.y*BLOCK_SIZE+n]);
            tempB.init(Bs[n*BLOCK_SIZE+threadIdx.x]);
            CValue += tempA.asFloat() * tempB.asFloat();
        }
...
__global__ void number_matmul_block_lin_shared_trans(float* A, float* B, float* C,
                           int ARows, int ACols, int BRows,
                           int BCols, int CRows, int CCols, Number::TypeInfo metaA, Number::TypeInfo metaB)
{
    float CValue = 0;
...
__global__ void number_matmul_block_lin_shared_batch(const float* A, const float* B, float* C,
                             int ARows, int ACols,
                             int BRows, int BCols,
                             int CRows, int CCols, int group, Number::TypeInfo metaA, Number::TypeInfo metaB)
{
    float CValue = 0;
...
// A: MxK, B: KxN, C: MxN
__global__ void number_gemmStridedBatched(
           const int M, const int N, const int K,
           const float* A, const int ARows, const int nPerBatchA,
           const float* B, const int BRows, const int nPerBatchB,
                 float* C, const int CRows, const int nPerBatchC,
           Number::TypeInfo metaA, Number::TypeInfo metaB)
{
    float CValue = 0;
